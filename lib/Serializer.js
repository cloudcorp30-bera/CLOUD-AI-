...
// Custom simple in-memory store implementation
function makeInMemoryStore(options = {}) {
    ...
    const store = {
        ...
        loadMessage: function(jid, id, sock = null) {
            if (!this.messages[jid]) return null;
            const msg = this.messages[jid].find(m => m.key?.id === id);
            
            // If message not found and sock is provided, try to fetch from WhatsApp
            if (!msg && sock) {
                // This is a fallback - in real implementation you'd fetch from WhatsApp
                return null;
            }
            return msg;
        },
        
        // Save a message to store
        saveMessage: function(message) {
            ...
            // Keep only last 100 messages per chat to prevent memory issues
...
function serialize(m, sock, logger) {
  ...
  sock.ev.on('contacts.update', update => {
    if (store && store.updateContacts) {
      store.updateContacts(update);
    }
  });
  ...
  // Save message to store when serializing
  if (m && m.key) {
    store.saveMessage(m);
  }
...
export { decodeJid, serialize, store };
